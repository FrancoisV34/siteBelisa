...
export async function onRequestPost({ request, env }) {
  try {
    ...
    const result = await env.DB.prepare(
      `INSERT INTO posts (author_id, slug, title, content_html, excerpt, cover_image, meta_description, og_image, image_alt, status, published_at, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).bind(
      auth.user.id, slug, title, contentHtml, excerpt, coverImage,
      metaDescription, ogImage, imageAlt, status, publishedAt, now, now
    ).run()
